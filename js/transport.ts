...
import { filter, inflate_long, read_rng } from './src/utilities';
...
export class SSHyClientTransport {
  ...
  handler_table = {
    ...
    6: function(self, m) {
      var service = new SSHyClientMessage(m.slice(1)).get_string();
...
// Sets up the keys and ciphers that the parceler will use
  activate_encryption() {
    ...
    this.parceler.outbound_cipher = new SSHyClientCrypto.AES(this.parceler.outbound_enc_key,
      SSHyClientDefines.AES_CTR,
      this.parceler.outbound_enc_iv,
      new SSHyClientCrypto.counter(this.parceler.block_size * 8, inflate_long(this.parceler.outbound_enc_iv)));
    ...
    this.parceler.inbound_cipher = new SSHyClientCrypto.AES(this.parceler.inbound_enc_key,
      SSHyClientDefines.AES_CTR,
      this.parceler.inbound_enc_iv,
      new SSHyClientCrypto.counter(this.parceler.block_size * 8, inflate_long(this.parceler.inbound_enc_iv)));
